...
for i in profesores:
    ...
    if i.horas_grupo <= 0:
        print(f"Profesor {i.nombre} tiene horas de grupo invalidas")
    if i.materia not in materias:
        materias[i.materia] = 1
    else:
        materias[i.materia] += 1
    if (
        i.total - (i.horas_grupo + i.horas_servicio + (1 if i.tutoria != None else 0))
        != 0
    ):
    ...
    for prof in profesores:
        if grupo in prof.grupos:
            if horas_requeridas[prof.materia] > prof.horas_grupo / len(prof.grupos):
                print(
                    f"El maestro {prof.nombre} no puede cumplir sus horas requeridas con el total de sus grupos, pues la matera {prof.materia} para el grupo {grupo} requiere de {horas_requeridas[prof.materia]} horas y con {len(prof.grupos)} grupos y con {prof.horas_grupo} de horas de grupo serian un total de {prof.horas_grupo/len(prof.grupos)} horas por grupo"
                )
